...
import { evidenceBanner } from "@/lib/query-presentation";
...
function EvidenceNotice({ message }: { message: ChatMessage }) {
  const banner = evidenceBanner(message.confidence ?? "none", message.citations?.length ?? 0);
  if (!banner) return null;
  return <div className={`mt-3 rounded-md border px-3 py-2 text-xs ${banner.className}`}>{banner.label}</div>;
}
